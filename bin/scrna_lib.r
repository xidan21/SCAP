...
tmm <- function(input_matrix, ...) {
        ...
        group <- c(replicate(ncol(input_matrix), "A"))
        
        f <- DGEList(counts = e, group=group)
        
        f = calcNormFactors(f, method="TMM")
        scale <- f$samples$lib.size * f$samples$norm.factors # normalized library sizes
        rescale <- f$samples$lib.size/ mean(scale)
        normCounts <- t(t(e)/rescale)
...
convert.to.color_2 <- function(x,colscale,col.range=NULL){
  x.range<-range(na.omit(x))
  by=0.1
  if (is.null(col.range)){
    by=10
